...
namespace MoonSharp.Interpreter
{
    ...
    public static class ModuleRegister
    {
        ...
        public static Table RegisterModuleType(this Table gtable, Type t)
        {
                ...
                if (mi.GetCustomAttributes(typeof (MoonSharpModuleMethodAttribute), false).Length > 0)
                {
                    var attr =
                        (MoonSharpModuleMethodAttribute)
                            mi.GetCustomAttributes(typeof (MoonSharpModuleMethodAttribute), false).First();
                    ...
                    var func =
                        (Func<ScriptExecutionContext, CallbackArguments, DynValue>)
                            Delegate.CreateDelegate(typeof (Func<ScriptExecutionContext, CallbackArguments, DynValue>),
                                mi);
            ...
            foreach (
                var fi in
                    t.GetFields(BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic)
                        .Where(_mi => _mi.GetCustomAttributes(typeof (MoonSharpModuleMethodAttribute), false).Length > 0)
                )
            {
                var attr =
                    (MoonSharpModuleMethodAttribute)
            ...
            foreach (
                var fi in
                    t.GetFields(BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic)
                        .Where(
                            _mi => _mi.GetCustomAttributes(typeof (MoonSharpModuleConstantAttribute), false).Length > 0)
                )
            {
                var attr =
        ...
        private static void RegisterScriptField(FieldInfo fi, object o, Table table, Type t, string name)
        {
            if (fi.FieldType != typeof (string))
            {
                throw new ArgumentException(string.Format(
                    "Field {0} does not have the right type - it must be string.", name));
            }
        ...
        {
            var attr =
                (MoonSharpModuleAttribute) t.GetCustomAttributes(typeof (MoonSharpModuleAttribute), false).First();
